// assets/js/loadSection.js (VERSIÓN MODULARIZADA COMPLETA)

// ====================================================================
// === FUNCIONES AUXILIARES (Definidas FUERA del IIFE para ser reutilizables) ===
// ====================================================================
...
// ====================================================================
// === FUNCIÓN CENTRAL DE APLICACIÓN DE DATOS (Exportada para el Preview) ===
// ====================================================================

/**
 * Función principal para aplicar datos del CMS a los elementos del DOM.
 * Contiene toda la lógica de mapeo que antes estaba dentro del fetch.
 * @param {object} data - Datos del Front Matter del CMS.
 * @param {object} mapping - Mapa de clave CMS a ID de elemento.
 * @param {Document} domContext - El contexto del DOM (document principal o el iframe del preview).
 */
export function applyDataToDOM(data, mapping, domContext = document) {

    for (const key in mapping) {
        const elementId = mapping[key];
        // Buscamos el elemento dentro del contexto del DOM (document o iframe)
        const el = domContext.getElementById(elementId);
        if (!el || typeof data[key] === 'undefined') continue;

        const rawValue = String(data[key]).trim();

        // ---------------------------------------------------------------------
        // INICIO DE LA LÓGICA DE APLICACIÓN (IDÉNTICA A TU CÓDIGO ORIGINAL)
        // ---------------------------------------------------------------------

        // Manejo específico: header background mode (image | color | none)
        if (key === 'header_background_mode') {
            const header = domContext.getElementById('header-section');
            const img = domContext.getElementById('header-background');
            ...
            if (mode === 'image') {
                ...
                if (src && header) {
                    // Nota: En el Live Preview, la carga de imagen se simplifica.
                    // Aquí se mantiene tu lógica original de preloading/onerror para el sitio en vivo.
                    const pre = new Image();
                    pre.src = src;
                    pre.onload = () => {
                        if (header) {
        ...
        // PRIORIDAD: regla específica para el botón del header
        if (key === 'header_button_bg_color') {
            ...
            // Aquí leemos el color del texto del botón de los datos y lo aplicamos a los SPANs hijos.
            ...
            if (textColor) {
                ...
                spans.forEach(span => {
                 ...
                 const spans = el.querySelectorAll('span');
                 spans.forEach(span => {
                    span.style.color = ''; // Limpiar si no hay valor
                });
            }
            continue;
        }

        // Manejo genérico de enlaces
        if (el.tagName === 'A') {
        ...
        if (key === 'header_title_color' || key === 'header_subtitle_color' || /^services_subtitle_color(_en)?$/.test(key)) {
            ...
            if (color) {
                ...
                const enEl = domContext.getElementById(elementId + '-en');
        ...
        // SPANs: usar siempre textContent
        if (el.tagName === 'SPAN') {
        ...
        // Por defecto: contenido rico (Markdown).
        if (/^[\w- ]+$/.test(rawValue) && !rawValue.includes('\n')) {
            el.textContent = rawValue;
        } else {
            // marked.parse solo funciona si la biblioteca `marked` está definida
...
// ====================================================================
// === LÓGICA DEL SITIO WEB EN VIVO (Ejecución al cargar la página) ===
// ====================================================================

(function() {
    // Función que inicia la carga, obtiene los datos y llama a la lógica central
    function loadSection(sectionName, mapping) {
        fetch(`content/${sectionName}.md`)
            ...
            .then(text => {
                ...
                // Llama a la lógica central exportada, usando 'document' como contexto
                applyDataToDOM(data, mapping, document);
            })
            .catch(err => {
    ...
    const HEADER_MAPPING = {
        ...
        // header_button_text_color se aplica internamente en applyDataToDOM
    };
    ...
    // Inicialización del sitio en vivo
    loadSection("header", HEADER_MAPPING);
    loadSection("about", ABOUT_MAPPING);
    loadSection("portfolio", PORTFOLIO_MAPPING);
    loadSection("services", SERVICES_MAPPING);
    loadSection("callout", CALLOUT_MAPPING);
    loadSection("cta", CTA_MAPPING);
    loadSection("footer", FOOTER_MAPPING);
